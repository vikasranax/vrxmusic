...
export function startEngine() {
  engineStarted = true;
  if (!ready) { init().then(() => { try { player.playVideo(); } catch {} flushPending(); }); return; }
  flushPending();
  try { player.setVolume(S().muted ? 0 : S().volume); player.playVideo(); } catch {}
}

/** 1-click channel switch (loads instantly; plays if engine started) */
export function playChannel(channelId) {
  ...
  const doLoad = () => {
    ...
    if (engineStarted) setTimeout(() => { try { player.playVideo(); } catch {} }, 400);
  };
...
export function playUrl(url) {
  ...
  if (playlistId) {
    mode = 'playlist';
    const run = () => { S().setStatus('loading'); player.loadPlaylist({ list: playlistId }); try { player.setShuffle(true); } catch {} };
    ...
    if (!ready) pending = loadCurrent; else loadCurrent();
  } else {
