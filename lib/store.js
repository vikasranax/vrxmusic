...
export const useStore = create(
  persist(
    (set, get) => ({
      ...
      currentTrack: { title: 'Aetheria — continuous stream', badge: 'Live Stream', videoId: null },
      ...
      setSyncKey: (k) => {
        set((s) => {
          const exists = s.knownKeys.find((x) => x.key === k);
          const updated = exists
            ? s.knownKeys.map((x) => (x.key === k ? { ...x, usedAt: Date.now() } : x))
            : [{ key: k, label: '', usedAt: Date.now() }, ...s.knownKeys];
          return { syncKey: k, knownKeys: updated.slice(0, 10) };
        });
      },
      setKeyLabel: (key, label) => set((s) => ({
        knownKeys: s.knownKeys.map((x) => (x.key === key ? { ...x, label } : x)),
      })),
      removeKnownKey: (key) => set((s) => ({
        knownKeys: s.knownKeys.filter((x) => x.key !== key),
        syncKey: s.syncKey === key ? '' : s.syncKey,
      })),
      ...
      pullFromCloud: async () => {
        ...
        const { syncKey } = get();
        if (!syncKey) return;
        const { data, error } = await supabase.from('lists').select('*').eq('sync_key', syncKey).single();
    ...
    }),
    {
      ...
      partialize: (s) => ({
        ...
        customBg: s.customBg, bgMode: s.bgMode, syncKey: s.syncKey, knownKeys: s.knownKeys,
      }),
    }
  )
);