...
import { Search, X, Plus, Check, Star, Trash2, Clock, Calendar, Info, Film, Tv } from 'lucide-react';
...
export default function MoviePanel() {
  ...
  const [kind, setKind] = useState('movie'); // 'movie' | 'tv'  ← series list is back
  ...
  // trending (movies or series)
  useEffect(() => {
    if (!open || tab !== 'search' || query.trim()) return;
    let alive = true;
    setLoading(true);
    const p = kind === 'tv' ? tmdb.trendingTv() : tmdb.trending();
    const map = kind === 'tv' ? tmdb.toMiniTv : tmdb.toMini;
    p.then((r) => alive && setRows((r.results || []).map(map)))
      .catch(() => {})
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      setLoading(true);
      try {
        const r = kind === 'tv' ? await tmdb.searchTv(query) : await tmdb.search(query);
        const map = kind === 'tv' ? tmdb.toMiniTv : tmdb.toMini;
        setRows((r.results || []).map(map));
      } catch {}
      setLoading(false);
    }, 300);
  ...
  const openDetail = async (m) => {
    ...
    try {
      ...
      const [data, provData] = await Promise.all([
        isTv ? tmdb.detailsTv(m.id) : tmdb.details(m.id),
        isTv ? tmdb.tvProviders(m.id) : tmdb.providers(m.id),
      ]);
      setDetailData(data);
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-white/5 px-5 py-4">
              ...
              <button onClick={() => setOpen(false)} className="text-[#4a5568] transition hover:text-ink"><X size={18} /></button>
            </div>
            ...
            {/* Search + Movies/Series toggle */}
            {tab === 'search' && (
              <div className="border-b border-white/5 px-5 py-3">
                <div className="flex items-center gap-2">
                  <div className="flex shrink-0 gap-1 rounded-lg bg-white/5 p-1">
                    <button
                      onClick={() => { setKind('movie'); setQuery(''); }}
                      className={`flex items-center gap-1 rounded-md px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider transition ${kind === 'movie' ? 'bg-[#34e1d6] text-[#04181a]' : 'text-[#4a5568] hover:text-[#8a97a6]'}`}>
                      <Film size={11} /> Movies
                    </button>
                    <button
                      onClick={() => { setKind('tv'); setQuery(''); }}
                      className={`flex items-center gap-1 rounded-md px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider transition ${kind === 'tv' ? 'bg-[#34e1d6] text-[#04181a]' : 'text-[#4a5568] hover:text-[#8a97a6]'}`}>
                      <Tv size={11} /> Series
                    </button>
                  </div>
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      placeholder={kind === 'tv' ? 'Search series…' : 'Search movies…'}
            ...
            <div className="thin-scroll flex-1 overflow-y-auto px-5 py-4">
              {tab === 'search' && (
                <>
                  <p className="mb-3 text-[10px] font-semibold uppercase tracking-wider text-[#4a5568]">
                    {kind === 'tv' ? 'Trending series this week' : 'Trending this week'}
                  </p>
  ...
  return (
    <button onClick={onClick} className="flex w-full items-center gap-3 rounded-lg bg-white/[0.02] p-2 text-left transition hover:bg-white/[0.04]">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[10px] text-[#4a5568]">{m.year}{m.mediaType === 'tv' ? ' · Series' : ''}</p>
      </div>
...
function DetailModal({ detail, onClose, detailData, detailProv, inList, addToWatchlist, markWatched }) {
  ...
  const runtimeMin = detailData ? (Array.isArray(detailData.episode_run_time) ? detailData.episode_run_time[0] : detailData.runtime) : null;
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="thin-scroll relative max-h-[90dvh] w-full max-w-lg overflow-y-auto rounded-2xl border border-white/10 bg-[#0c0f14] shadow-2xl"
      >
        {/* Backdrop */}
        ...
        <div className="relative -mt-16 px-6 pb-6">
          <div className="flex gap-4">
            <img src={img(detail.poster, 'w342')} alt={detail.title} className="h-32 w-24 flex-none rounded-lg object-cover shadow-lg sm:h-40" />
            <div className="mt-16 flex-1">
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-bold text-ink sm:text-2xl">{detail.title}</h2>
                {isTv && <span className="rounded bg-white/10 px-1.5 py-0.5 text-[9px] uppercase tracking-wider text-[#8a97a6]">Series</span>}
              </div>
              <div className="mt-1.5 flex flex-wrap items-center gap-3 text-xs text-[#8a97a6]">
                ...
                {isTv && detailData?.number_of_seasons ? (
                  <span className="flex items-center gap-1"><Tv size={11} />{detailData.number_of_seasons} Season{detailData.number_of_seasons > 1 ? 's' : ''}</span>
                ) : (runtimeMin > 0 && <span className="flex items-center gap-1"><Clock size={11} />{Math.floor(runtimeMin / 60)}h {runtimeMin % 60}m</span>)}
