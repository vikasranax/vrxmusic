'use client';
import { Waves, Droplet, TreePine, Mountain, Gem, CloudRain, Sparkles, Image as ImageIcon, X } from 'lucide-react';
...
export default function LeftRail() {
  ...
  const setImageBg = () => {
    ...
    if (url && url.trim()) setCustomBg(url.trim());
  };
  ...
  return (
    <div className="absolute left-6 top-1/2 z-20 flex -translate-y-1/2 flex-col gap-4">
      {SCENE_ORDER.map((id, i) => {
        ...
        const on = id === sceneId && bgMode === 'scene';
        return (
          <button key={id} title={id} onClick={() => setSceneId(id)}
      ...
      {bgMode === 'image' ? (
        <button title="Back to 3D scenes" onClick={clearImage}
          className="grid h-[46px] w-[46px] place-items-center rounded-full border border-accent/40 bg-accent/10 text-accent backdrop-blur transition hover:bg-accent/20">
          <X size={18} />
        </button>
      ) : (
        <button title="Set image background" onClick={setImageBg}
          className="grid h-[46px] w-[46px] place-items-center rounded-full border border-white/10 bg-[rgba(8,11,16,.5)] text-dim backdrop-blur transition hover:text-ink hover:border-white/20">
          <ImageIcon size={18} />
        </button>
      )}
    </div>
